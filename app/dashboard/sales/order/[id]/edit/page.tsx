...
export default async function EditSalesOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [order, members, openCpos] = await Promise.all([
  ...
  return <EditSalesOrderClient order={order} members={members} currentUserName={session.user.name ?? ""} openCpos={openCpos} />;
}
