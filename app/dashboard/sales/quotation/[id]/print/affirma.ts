...
function wrap(text: string, font: PDFFont, size: number, maxW: number): string[] {
  ...
  for (const word of words) {
    ...
    if (font.widthOfTextAtSize(test, size) <= maxW) {
      ...
      // Hard-break a word that is wider than maxW
      let remaining = word;
...
export async function generateQuotationAffirma(data: Data): Promise<Uint8Array> {
  ...
  for (let pi = 0; pi < pageGroups.length; pi++) {
    ...
    for (const rowIdx of pageItems) {
      ...
      if (showCode) {
        ...
        page.drawText(trunc(item.productCode, fontB, FS_CODE - 1, C_DESC - TABLE_PAD * 2), {
          x: X_DESC + TABLE_PAD, y: dy, size: FS_CODE - 1, font: fontB, color: accentColor,
        });
        dy -= CODE_LINE_H;
      }
    ...
    if (isLast) {
      ...
      // ── Acceptance block (finalized quotations only) ──────────────────────
      if (q.status === "final") {
        curY -= 10;
