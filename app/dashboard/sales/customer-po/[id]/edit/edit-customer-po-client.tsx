...
export function EditCustomerPoClient({
  ...
  const [salesPersonName,        setSalesPersonName]        = useState(cpoAny.salesPersonName ?? "");
  const [salesPersonInherited,   setSalesPersonInherited]   = useState(cpoAny.salesPersonName ?? "");
  const [associateSalesPersons,  setAssociateSalesPersons]  = useState<{ id: string; name: string }[]>(
    () => (cpoAny.associateSalesPersons as { id: string; name: string }[] | null) ?? []
  );
